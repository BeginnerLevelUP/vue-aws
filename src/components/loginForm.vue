    ...
    async submitForm() {
      ...
      if (this.isValid(this.formData)) {
        ...
        try {
          ...
          if (data?.token) {
            loginRest(this.formData.userName, this.formData.userName);
            console.log('User Logged in successfully:', data);
            localStorage.setItem('id_token', data?.token);
            this.$router.go(0)
          } else {
            console.error('Error Logging User:', data);
          }
        } catch (err) {
  ...
  <dialog id="loginModal" class="modal">
    ...
    <form @submit.prevent="submitForm" method="dialog">
      ...
      <label className="input input-bordered flex items-center gap-2">
        ...
        <input type="password" className="grow" placeholder="Password" v-model="formData.userPassword" />
      </label>
